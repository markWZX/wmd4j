...
public class MinimalCostFlow {
	...
	public long compute(long[] e, List<List<Edge>> c, List<List<Edge0>> x) {

		Integer numNodes = e.length;

		List<Integer> nodesToQ = new ArrayList<Integer>(numNodes);
		List<List<Edge1>> rCostForward = new ArrayList<List<Edge1>>(numNodes);
		List<List<Edge2>> rCostCapBackward = new ArrayList<List<Edge2>>(
				numNodes);

		for (int i = 0; i < numNodes; i++) {
			nodesToQ.add(0);
			rCostForward.add(new ArrayList<Edge1>(c.get(i).size()));
			...
			rCostCapBackward.add(new ArrayList<Edge2>(c.get(i).size()));
		}

		long U = 0;
		long delta;

		List<Long> d = new ArrayList<Long>(numNodes);
		List<Integer> prev = new ArrayList<Integer>(numNodes);
		...
		while (true) {
			...
			int[] l = new int[1];
			computeShortestPath(d, prev, k, rCostForward, rCostCapBackward, e,
					l, nodesToQ, numNodes);
	...
	private void computeShortestPath(List<Long> d, List<Integer> prev,
		...
		List<Edge3> Q = new ArrayList<Edge3>(numNodes);
		for (int i = 0; i < numNodes; i++) {
			Q.add(new Edge3());
		}
	...
	private void heapDecreaseKey(List<Edge3> Q, List<Integer> nodesto_Q, int v,
			long alt) {
	...
	private void heapRemoveFirst(List<Edge3> Q, List<Integer> nodesto_Q) {
		swapHeap(Q, nodesto_Q, 0, Q.size() - 1);
		Q.remove(Q.size() - 1);
		heapify(Q, nodesto_Q, 0);
	}

	private void heapify(List<Edge3> Q, List<Integer> nodesto_Q, int i) {
		do {
			// TODO: change to loop
			int l = LEFT(i);
			int r = RIGHT(i);
	...
	private void swapHeap(List<Edge3> Q, List<Integer> nodesToQ, int i, int j) {
		Edge3 tmp = Q.get(i);
		Q.set(i, Q.get(j));
		Q.set(j, tmp);
