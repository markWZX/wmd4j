...
public class EarthMovers {
	...
	public double distance(double[] P, double[] Q, double[][] C,
	...
	public long distance(long[] Pc, long[] Qc, long[][] C, long extraMassPenalty) {
		...
		List<List<Edge>> c = new ArrayList<List<Edge>>(b.length);
		...
		List<List<Edge>> cc = new ArrayList<List<Edge>>(bb.length);
		List<List<Edge0>> flows = new ArrayList<List<Edge0>>(bb.length);

		for (int i = 0; i < bb.length; i++) {
			cc.add(new LinkedList<Edge>());
			flows.add(new ArrayList<Edge0>(bb.length * 2));
		}
